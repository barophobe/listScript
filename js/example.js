$(function() {
  ...
  $removeItemButton = $('#removeItemButton');          // Cache button to show form
  ...
  $('#showForm').on('click', function() {        // When click on add item button
    ...
    $('.complete').each(function( index ) {
      ...
      // $('li[class=complete]').animate({                  // If so, animate opacity + padding
      //   opacity: 0.0,
      //   paddingLeft: '+=180'
  ...
  $list.on('click', 'li', function() {
    ...
    if (complete === true) { 
      // console.log('complete');          // Check if item is complete
      var inComplete = $this.removeClass('complete');
      ...
      // });
       ...
       $this.addClass('complete')                      // Otherwise indicate it is complete
      // item = $this.text();             // Get the text from the list item
      // $this.remove();                  // Remove the list item
      // $list                            // Add back to end of list as complete
      //   .append('<li class=\"complete\">' + item + '</li>')
